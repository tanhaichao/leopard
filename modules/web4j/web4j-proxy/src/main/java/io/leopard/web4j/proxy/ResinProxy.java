...
import io.leopard.burrow.util.MapUtil;
...
public class ResinProxy {
	private static final Log logger = LogFactory.getLog(ResinProxy.class);
	...
	protected static String getResinIp(HttpServletRequest request) {
		...
		if (StringUtils.isEmpty(resinIp)) {
	...
	protected static String getUrl(String resinIp, String uri, HttpServletRequest request) {
		StringBuilder sb = new StringBuilder();
		...
		if (MapUtil.isNotEmpty(map)) {
			...
			for (Entry<String, String[]> entry : map.entrySet()) {
				...
				for (String value : values) {
					...
					queryString.append(name).append("=").append(StringUtil.urlEncode(value));
				}
			}
			if (queryString.length() > 0) {
